...
export class ContractsController {
  ...
  @Get('my-bids')
  @ApiOperation({ summary: 'Получить мои заявки на контракты' })
  @ApiResponse({ status: 200, description: 'Список моих заявок' })
  async getMyBids(@Request() req: any) {
    return this.contractsService.getUserBids(req.user.userId);
  }
  ...
  @Get(':id/bids')
  @ApiOperation({ summary: 'Получить все заявки на контракт' })
  ...
  @Post(':contractId/bids/:bidId/accept')
  ...
  @ApiResponse({ status: 200, description: 'Контракт завершен, вознаграждение начислено' })
  @ApiResponse({ status: 400, description: 'Контракт не в работе' })
  @ApiResponse({ status: 404, description: 'Контракт не найден' })
  async completeContract(@Param('id') contractId: string, @Request() req: any) {
