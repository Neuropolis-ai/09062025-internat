...
## 6. 🔔 Страница уведомлений (/notifications) - 🔥 **ТЕКУЩИЙ ПРИОРИТЕТ**

### 🔧 Что нужно создать с нуля:
- [ ] **КРИТИЧНО**: Подключить к реальному API уведомлений
- [ ] Отображение всех уведомлений системы в таблице
- [ ] Создание новых уведомлений через модальное окно
- [ ] Настройка целевых групп получателей (классы, коттеджи, роли)
- [ ] Планирование отправки уведомлений (немедленно, по расписанию)
- [ ] Статистика прочтения уведомлений
- [ ] Шаблоны уведомлений
- [ ] Типы уведомлений (информационные, срочные, объявления)
- [ ] Статусы уведомлений (черновик, отправлено, прочитано)
...
- [ ] `PATCH /api/v1/notifications/:id` - обновить уведомление
- [ ] `DELETE /api/v1/notifications/:id` - удалить уведомление
- [ ] `POST /api/v1/notifications/:id/send` - отправить уведомление
- [ ] `GET /api/v1/notifications/:id/stats` - статистика уведомления
- [ ] `GET /api/v1/notifications/templates` - шаблоны уведомлений
